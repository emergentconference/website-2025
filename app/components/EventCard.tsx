...
export function EventCard({
  ...
  return (
    <div
      className={`relative mt-2 rounded-sm bg-gray-100 overflow-hidden transition-all duration-300 ${featuredEvent
    ...
    >
      ...
      <div
        className={`relative transition-all duration-300 ease-in-out overflow-hidden ${expanded
                ...
                <div className="border-l-3 border-gray-700 ml-0.5 pl-4 mb-6 grid grid-cols-[repeat(auto-fit,minmax(110px,1fr))] gap-4 items-center">
                  {speakers &&
                    speakers.map((logo) => (
                      <div
                        key={logo.name}
                        className="relative h-8 sm:h-10 md:h-12 w-full max-w-[200px] mx-auto"
                      >
                        <Image
                          src={logo.src}
                          alt={logo.name}
              ...
              <>
                ...
                <div className="border-l-3 border-gray-700 ml-0.5 pl-4 mb-6 grid grid-cols-[repeat(auto-fit,minmax(110px,1fr))] gap-4 items-center">
                  {sponsors.map((logo) => (
                    <div
                      key={logo.name}
                      className="relative h-6 sm:h-8 w-full max-w-[200px] mx-auto"
                    >
                      <Image
                        src={logo.src}
                        alt={logo.name}
